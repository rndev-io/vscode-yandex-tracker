...
Object.defineProperty(exports, "__esModule", { value: true });
class Tracker {
    constructor(client, host, token, orgId = '') {
        this._client = client;
        this.host = host;
        this.token = token;
        this.orgId = orgId;
        this.frontByHost = new Map([
    ...
    me() {
        return __awaiter(this, void 0, void 0, function* () {
            const response = yield this.client().get('/myself');
...
class User {
    ...
    raw() {
        return __awaiter(this, void 0, void 0, function* () {
            const response = yield this.client.get(`/users/${this.uid}`);
...
class Issues {
    ...
    search(query) {
        return __asyncGenerator(this, arguments, function* search_1() {
            ...
            const perPage = 50;
            while (true) {
                const response = yield __await(this.client.post('/issues/_search', { query: query }, { params: { page: page, perPage: perPage } }));
                yield __await(yield* __asyncDelegator(__asyncValues(response.data.map((item) => {
...
class Issue {
    constructor(client, number, title = '') {
        this.client = client;
        this.num = number;
        this.dsc = title;
    }
    ...
    description() {
        return __awaiter(this, void 0, void 0, function* () {
            return this.dsc === '' ? (yield this.raw()).summary : this.dsc;
        });
    }
    raw() {
        return __awaiter(this, void 0, void 0, function* () {
            const response = yield this.client.get(`/issues/${this.num}`);
            return response.data;
        });
    }
    comments() {
...
exports.Issue = Issue;
class Comments {
    constructor(client, issueNumber) {
        this.client = client;
        this.issueNumber = issueNumber;
    }
    all() {
        return __awaiter(this, void 0, void 0, function* () {
            const response = yield this.client.get(`/issues/${this.issueNumber}/comments`);
...
class Comment {
    constructor(client, issueNumber, commentId, raw) {
        this.client = client;
        this.issueNumber = issueNumber;
    ...
    raw() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const response = yield this.client.get(`/issues/${this.issueNumber}/comments/${this.commentId}`);
            return response.data;
        });
    }
}
...
Object.defineProperty(exports, "__esModule", { value: true });
const keychain_1 = __webpack_require__(/*! ./keychain */ "./src/keychain.ts");
...
class SecretNotFound extends BaseError {
}
exports.SecretNotFound = SecretNotFound;
class Credentials {
    constructor(extensionId, host) {
        this.extensionId = extensionId;
        this.host = host;
    }
    token() {
        return __awaiter(this, void 0, void 0, function* () {
            return yield keychain_1.keychain.getPassword(this.extensionId, this.host);
        });
    }
    save(token) {
        return __awaiter(this, void 0, void 0, function* () {
            yield keychain_1.keychain.setPassword(this.extensionId, this.host, token);
        });
    }
    clean() {
        return __awaiter(this, void 0, void 0, function* () {
            yield keychain_1.keychain.deletePassword(this.extensionId, this.host);
        });
    }
}
...
const issuesTree_1 = __webpack_require__(/*! ./views/issuesTree */ "./src/views/issuesTree.ts");
const api_1 = __webpack_require__(/*! ./api */ "./src/api.ts");
const issuePanel_1 = __webpack_require__(/*! ./views/issuePanel */ "./src/views/issuePanel.ts");
const resource_1 = __webpack_require__(/*! ./resource */ "./src/resource.ts");
const axios_1 = __webpack_require__(/*! axios */ "./node_modules/axios/index.js");
function activate(context) {
    return __awaiter(this, void 0, void 0, function* () {
        const extensionId = 'vscode-yandex-tracker';
        const trackerHost = vscode.Uri.parse(vscode.workspace.getConfiguration(extensionId).get('host') || '');
        const orgID = vscode.workspace.getConfiguration(extensionId).get('organizationId') || '';
        const customQuery = vscode.workspace.getConfiguration(extensionId).get('query') || '';
        const resource = new resource_1.Resource(context);
        const credentials = new credentials_1.Credentials(`${extensionId}`, trackerHost.authority);
        const token = yield credentials.token();
        if (token === null) {
            vscode.window.showInformationMessage(`Set OAuth token for host ${trackerHost.toString()}`, 'Setup Token').then((value) => {
                if (value !== undefined) {
                    vscode.commands.executeCommand(`${extensionId}.setOAuthToken`);
                }
            });
        }
        const tracker = new api_1.Tracker(axios_1.default.create(), trackerHost.toString(), token || '', orgID);
        const issuePanel = new issuePanel_1.IssuePanel(context, tracker);
        const assignToMeView = new issuesTree_1.IssuesProvider(tracker, resource, 'Resolution: empty() and Assignee: me()');
        const followedByMe = new issuesTree_1.IssuesProvider(tracker, resource, 'Resolution: empty() and Followers: me()');
        const custom = new issuesTree_1.IssuesProvider(tracker, resource, customQuery);
        ...
        vscode.commands.registerCommand(`${extensionId}.setOAuthToken`, () => __awaiter(this, void 0, void 0, function* () {
            const tkn = yield vscode.window.showInputBox({ placeHolder: 'Set OAuth token' });
            if (tkn === undefined) {
                vscode.window.showErrorMessage('Token can\'t be empty');
                return;
            }
            yield credentials.save(tkn);
            vscode.window.showInformationMessage(`Token saved for host ${trackerHost.authority}`, 'Reload window').then((value) => {
                if (value !== undefined) {
                    vscode.commands.executeCommand('workbench.action.reloadWindow');
                }
            });
        }));
        vscode.commands.registerCommand(`${extensionId}.openIssue`, (number) => __awaiter(this, void 0, void 0, function* () {
            if (number === undefined) {
                number = yield vscode.window.showInputBox({ placeHolder: 'Enter issue number' });
                if (number === undefined) {
                    vscode.window.showErrorMessage('ID must\'t be empty');
                    return;
                }
            }
            yield tracker.me(); // check that credentials is valid
...
class IssuePanel {
    ...
    show(ticketNumber) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!this.panel) {
                this.panel = vscode.window.createWebviewPanel('react', ticketNumber, column, { enableScripts: true, localResourceRoots: [vscode.Uri.file(path.join(this.context.extensionPath, 'media'))] });
            ...
            const comments = yield Promise.all((yield issue.comments().all()).map((c) => { return c.raw(); }));
            this.panel.webview.postMessage({
                command: 'issue',
                args: {
                    issue: yield issue.raw(),
...
class IssuesProvider {
    constructor(tracker, resource, query) {
    ...
    getChildren(element) {
        return __awaiter(this, void 0, void 0, function* () {
            let perPanel = 50;
            while (perPanel !== 0) {
                const issue = yield this.issues.next();
                if (issue === undefined) {
                    break;
                }
    ...
    getTreeItem(element) {
        return __awaiter(this, void 0, void 0, function* () {
            return {
                ...
                description: yield element.description(),
                tooltip: yield element.description(),
